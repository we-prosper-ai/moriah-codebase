...
import type { Teaching, ExtractionAudit, ExtractRequest, ExtractResponse, TranscriptMetadata } from './types';
import { insertTeachings, insertAudit } from './db';
...
function buildUserPrompt(req: ExtractRequest): string {
  ...
  if (meta) {
    ...
    if (meta.speakers?.length) parts.push(`Speakers: ${meta.speakers.join(', ')}`);
    if (meta.themes?.length) parts.push(`Detected Themes: ${meta.themes.join(', ')}`);
    if (meta.tags?.length) parts.push(`Tags: ${meta.tags.join(', ')}`);
    if (parts.length) context = `## Transcript Metadata\n${parts.join('\n')}\n\n`;
  }
...
export async function extractTeachings(req: ExtractRequest): Promise<ExtractResponse> {
...
export async function extractBatch(files: ExtractRequest[]): Promise<{
...
function parseAndValidateTeachings(raw: string, fileId: string, filename: string): Teaching[] {
...
function validateStringArray(val: unknown, field: string): string[] {
...
function validateDifficulty(val: unknown): 'beginner' | 'intermediate' | 'advanced' {
